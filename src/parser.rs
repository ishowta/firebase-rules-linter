...
struct Context<'a> {
    ...
    pub node_to_comments: &'a HashMap<usize, Vec<Comment>>,
}

fn attach_comments_to_next_node(
    node: &Node,
    source_code: &String,
    node_to_comments: &mut HashMap<usize, Vec<Comment>>,
) {
    let mut comments = vec![];

    // FIXME
    let no_expression_kinds = ["operator", "type", "params"];

    for child in node.children(&mut node.walk()) {
        if child.kind() == "comment" {
            println!("comment");
            comments.push(Comment {
                span: Span(child.range()),
                text: get_text(&child, source_code).to_owned(),
            });
        } else if !no_expression_kinds.contains(&child.kind()) {
            println!("{} {}", child.id(), get_text(&child, source_code));
            node_to_comments.insert(child.id(), comments);
            comments = vec![];
            attach_comments_to_next_node(&child, source_code, node_to_comments);
        }
    }
}
...
fn tree_sitter_parser_tree_to_ast(parser_tree: Tree, source_code: String) -> Ast {
    ...
    let mut node_to_comments = HashMap::new();
    node_to_comments.insert(root_node.id(), vec![]);
    attach_comments_to_next_node(&root_node, &source_code, &mut node_to_comments);

    println!("{:#?}", node_to_comments);
    ...
    Ast {
        tree: parse_code(
            ...
            &Context {
                ...
                node_to_comments: &node_to_comments,
            },
        ),
    }
}

fn get_text<'a>(n: &Node, source_code: &'a String) -> &'a str {
    n.utf8_text(source_code.as_bytes()).unwrap()
}
...
fn parse_literal(_node: &Node, context: &Context) -> Literal {
    ...
    match node.kind() {
        "string" => Literal::String(parse_string(get_text(&node, context.source_code))),
        "int" => Literal::Int(get_text(&node, context.source_code).parse::<i64>().unwrap()),
        "float" => Literal::Float(get_text(&node, context.source_code).parse::<f64>().unwrap()),
        "boolean" => Literal::Bool(
            get_text(&node, context.source_code)
                .parse::<bool>()
                .unwrap(),
        ),
        "list" => Literal::List(
                ...
                .map(|node| match node.kind() {
                    ...
                    "path_reference_string" => {
                        PathLiteral::PathExpressionSubstitution(Box::new(Expression {
                            ...
                            comments: context.node_to_comments[&node.id()].clone(),
...
fn parse_expression(node: &Node, context: &Context) -> Expression {
    println!("{} -> {}", node.id(), get_text(node, context.source_code));
    ...
    Expression {
        ...
        kind: match node.kind() {
            ...
            "identifier" => ExpressionKind::Variable(get_text(node, context.source_code).into()),
    ...
    Function {
        ...
        arguments: node
            ...
            .map(|node| Argument {
                ...
                span: Span(node.range()),
                comments: context.node_to_comments[&node.id()].clone(),
                name: get_text(&node, context.source_code).into(),
            })
            .collect(),
        let_bindings: node
...
fn parse_rule(node: &Node, context: &Context) -> Rule {
    Rule {
        ...
        permissions: node
            ...
            .map(|node| match get_text(&node, context.source_code) {
